...
# Could also be done with __init__ but yno I didnt for no reason.
...
class Crnafc(Task):
    ...
    * **discrim** - respond to input, set reward/punishment triggers on 
        target/distractor ports
    * **reinforcement** - deliver reward/punishment, end trial.
    ...
    Attributes:
        ...
        correction_trial (bool): If using correction trials, 
            last trial was a correction trial
        ...
        current_stage (int): As each stage is reached, update for 
            asynchronous event reference
    ...
    ## List of parameters needed to define the task
    # PARAMS is an ordered dict
    # Each key in PARAMS is the name of a parameter.
    # Each value in PARAMS is a dict with the following entries:
    #   'tag': string
    #       the human-readable name of the param
    #   'type': string, one of ['int', 'bool', 'list', 'sounds']
    #       the type of the param
    #   'depends': ?
    #   'values': ?
    #
    # When the task object is initialized, these params are passed to __init__
    # as kwargs, with the name of the param (i.e., the key in PARAMS) used
    # as the kwarg name, and its value as the kwarg value.
    #
    # The value of each param comes from the json file in the protocols
    # directory (I think?).
    
    # Initialize PARAMS as ordered dict
    PARAMS = odict()
    ...
    # TODO: Plots should take the default type, but options panel should 
    # be able to set - eg. corrects are done by rolling mean as default, 
    # but can be made points
    PLOT = {
        'data': {
            'target'   : 'point',
            'response' : 'segment',
    ...
    class TrialData(tables.IsDescription):
        # This class allows the Subject object to make a data table with 
        # the correct data types. You must update it for any new data you'd 
        # like to store
    ...
    def __init__(self, stage_block=None, stim=None, reward=50, req_reward=False,
                 punish_stim=False, punish_dur=100, 
                 correction=False, correction_pct=50.,
                 bias_mode=False, bias_threshold=20, current_trial=0, 
                 stim_light=True, **kwargs):
        """
        Args:
            stage_block (:class:`threading.Event`): Signal when task 
                stages complete.
            ...
            req_reward (bool): Whether to give a water reward in the 
                center port for requesting trials
            ...
            correction_pct (float):  (0-1), What proportion of trials should 
                randomly be correction trials?
            bias_mode (False, "thresholded_linear"): False, or some bias 
                correction type (see :class:`.managers.Bias_Correction` )
            bias_threshold (float): If using a bias correction mode, what 
                threshold should bias be corrected for?
            current_trial (int): If starting at nonzero trial number, which?
            stim_light (bool): Should the LED be turned blue while the 
                stimulus is playing?
            **kwargs:
        """
        ...
        # Set to true once the discrim stim has finished, used for punishing 
        # leaving C early
        #self.discrim_finished = False 
        
        self.discrim_playing = False
        
        # Keep track of stages so some asynchronous callbacks know when 
        # it's their turn
        ...
        ## We make a list of the variables that need to be reset each trial 
        # so it's easier to do so
